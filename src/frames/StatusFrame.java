 ...
 * @author nikol
 */
public class StatusFrame extends HBox {
    ...
    final Text caption = new Text("Status studenta");
    ...
    public StatusFrame() {
        ...
        TableColumn<Status, Integer> idColumn = new TableColumn<>("Id");
        idColumn.setCellValueFactory(new PropertyValueFactory<>("id"));
        idColumn.setMaxWidth(55);
        TableColumn<Status, String> infoColumn = new TableColumn<>("Info");
        infoColumn.setCellValueFactory(new PropertyValueFactory<>("info"));
        infoColumn.setMinWidth(190);
        statusTableView = new TableView<>();
        statusTableView.setMinWidth(162);
        statusTableView.getColumns().addAll(idColumn, infoColumn);
        statusTableView.setRowFactory(tv -> {
        ...
        this.getChildren().addAll(statusTableView);
        backButton.setOnAction(new OstaloAction());
        updateButton.setOnAction(e -> {
            try {
                checkU(idField.getText(), infoField.getText());
                controller.Connector.getInstance().updateStatus();
                controller.Connector.getInstance().refreshStatuss();
        ...
        insertButton.setOnAction(e -> {
            try {
                ...
                controller.Connector.getInstance().addStatus();
                controller.Connector.getInstance().refreshStatuss();
            } catch (emptyString | unallowedString ex) {
                JOptionPane.showMessageDialog(null, ex.getMessage());
            }
        ...
        deleteButton.setOnAction((event) -> {
            try {
                ...
                controller.Connector.getInstance().deleteStatus();
                controller.Connector.getInstance().refreshStatuss();
            } catch (emptyString | unallowedString ex) {
                JOptionPane.showMessageDialog(null, ex.getMessage());
            }
    ...
    public TableView<Status> getStatusTableView() {
        return statusTableView;
    }

    public void setNacinTableView(TableView<Status> statusTableView) {
    ...
    private void checkI(String id, String info) throws emptyString, unallowedString {
        ...
        for (Status p : Connector.getInstance().getStatuss()) {
            if ((p.getId() + "").equals(id)) {
                throw new unallowedString("Status sa zadatim id-om već postoji!");
            }
        }
    ...
    private void checkD(String id) throws emptyString, unallowedString {
        ...
        if (!pom) {
            throw new unallowedString("Status sa unešenim id-om ne postoji!");
        }
    }
